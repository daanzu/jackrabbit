...
function jackrabbit(url) {
  ...
  function bail(event, msg) {
    // TODO close any connections or channels that remain open
    connection = undefined;
    channel = undefined;
    // expected events: 'error', 'close'
    if (event) rabbit.emit(event, msg);
  }

  function onConnection(err, conn) {
    if (err) return bail('error', err);
    connection = conn;
    connection.on('close', bail.bind(this, 'close'));
    connection.on('error', bail.bind(this, 'error'));
